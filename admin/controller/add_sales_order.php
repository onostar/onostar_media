...
include "../classes/inserts.php";
    ...
    $rows = $get_item->fetch_details_cond('items', 'item_id', $item);
     if(gettype($rows) == 'array'){
        foreach($rows as $row){
            ...
            $cost = $row->cost_price;
            $markup = $row->markup;
            $commission = ($row->sales_price * ($row->commission/100));
    ...
    include "sales_details.php";
                ...
                $sell_item = new post_sales($item, $invoice, $quantity, $price, $price, $user_id, $sales_cost, $store, $sales_type, $customer, $date, $commission);
                $sell_item->add_sales();
                if($sell_item){
