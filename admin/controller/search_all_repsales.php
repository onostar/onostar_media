<?php
    ...
    $details = $get_revenue->fetch_details_2date2Con1Neg('sales', 'date(post_date)', $from, $to,  'store', $store, 'sales_status', 0);
    $n = 1;
?>
<h2>Sales Report between '<?php echo date("jS M, Y", strtotime($from)) . "' and '" . date("jS M, Y", strtotime($to))?>'</h2>
    ...
    <div class="search">
        ...
        <a class="download_excel" href="javascript:void(0)" onclick="convertToExcel('data_table', 'All Sales report')"title="Download to excel"><i class="fas fa-file-excel"></i></a>
    </div>
...
<?php
    ...
    $cashs = $get_cash->fetch_sum_2date2Cond1Neg('sales', 'commission', 'date(post_date)', 'store', 'sales_status', $from, $to, $store, 0);
    ...
    $amounts = $get_total->fetch_sum_2dateCond1Neg('sales', 'total_amount', 'store', 'date(post_date)', 'sales_status', $from, $to, $store, 0);
